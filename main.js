...
const CONFIG = {
  ...
  logDirtyMeta: String(process.env.LOG_DIRTY_META || "true") === "true",
};
...
async function handleQuote(quote, { route, meta = null }) {
  ...
  if (CONFIG.logDirtyMeta && meta) {
    console.log(
      `[OPPORTUNITY_META] eventType=${meta.eventType ?? "?"} pool=${meta.poolAddress ?? "?"} block=${meta.blockNumber ?? "?"} tx=${meta.txHash ?? "?"}`
    );
  }
  ...
  console.log({
    ...
    eventType: executionPayload.meta?.eventType ?? null,
    poolAddress: executionPayload.meta?.poolAddress ?? null,
    blockNumber: executionPayload.meta?.blockNumber ?? null,
    txHash: executionPayload.meta?.txHash ?? null,
  });
  ...
  const message = [
    ...
    meta?.eventType ? `Trigger: <b>${meta.eventType}</b>` : null,
    meta?.poolAddress ? `Pool: <code>${meta.poolAddress}</code>` : null,
    meta?.blockNumber ? `Block: <b>${meta.blockNumber}</b>` : null,
    meta?.txHash ? `Tx: <code>${meta.txHash}</code>` : null,
  ]
    .filter(Boolean)
    .join("\n");
...
function printBest() {
  ...
  if (best.meta) {
    console.log(`eventType=${best.meta.eventType ?? "?"}`);
    console.log(`poolAddress=${best.meta.poolAddress ?? "?"}`);
    console.log(`blockNumber=${best.meta.blockNumber ?? "?"}`);
    console.log(`txHash=${best.meta.txHash ?? "?"}`);
  }
