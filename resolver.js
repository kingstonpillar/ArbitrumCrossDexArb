...
function getV2Pair(pairAddress) {
  const key = pairAddress.toLowerCase();

  if (!v2PairCache.has(key)) {
    v2PairCache.set(key, new Contract(pairAddress, V2_PAIR_ABI, provider));
  }

  return v2PairCache.get(key);
}

function getV3Pool(poolAddress) {
  const key = poolAddress.toLowerCase();

  if (!v3PoolCache.has(key)) {
    v3PoolCache.set(
      key,
      new Contract(poolAddress, UNISWAP_V3_POOL_ABI, provider)
    );
  }

  return v3PoolCache.get(key);
}
...
export async function resolveV2Pair(factoryAddress, tokenA, tokenB) {
  return withRpcLimit(async () => {
    ...
    return pair === ZeroAddress ? null : pair.toLowerCase();
  });
}
...
export async function resolveUniswapV3Pool(tokenA, tokenB, fee) {
  return withRpcLimit(async () => {
    ...
    return pool === ZeroAddress ? null : pool.toLowerCase();
  });
}

export async function validateV2Pair(pairAddress) {
  return withRpcLimit(async () => {
    if (!pairAddress || pairAddress === ZeroAddress) return false;

    try {
      const pair = getV2Pair(pairAddress);
      ...
      return Boolean(token0 && token1 && token0 !== ZeroAddress && token1 !== ZeroAddress);
    } catch {
      return false;
    }
  });
...
export async function loadV3PoolMeta(poolAddress) {
  return withRpcLimit(async () => {
    ...
    try {
      const pool = getV3Pool(poolAddress);
